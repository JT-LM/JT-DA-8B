...
from mypybox.base import BasePyBox, BasePyBoxManager
from mypybox.schema import (
    ExecutionResponse,
    PyBoxOut,
)
...
class LocalPyBox(BasePyBox):
    def __init__(self, mkm: MultiKernelManager | None = None):
#         self.km = km
#         self.mkm = mkm
...
#         # self.client.wait_for_ready()
    ...
    def run(self, code: str, timeout: int = 60) -> PyBoxOut:
        ...
        res = self.__get_kernel_output(msg_id, timeout=timeout)
        # import time
        # time.sleep(3)
        # self.shutdown()
        return res
